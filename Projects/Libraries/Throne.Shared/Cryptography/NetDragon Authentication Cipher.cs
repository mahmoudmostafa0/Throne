...
namespace Throne.Shared.Cryptography
{
    ...
    /// </summary>
    public sealed unsafe class NetDragonAuthenticationCipher : INetworkCipher
    {
        ...
        private static readonly byte[] _iv; // The initialization vector.
        ...
        static NetDragonAuthenticationCipher()
        {
            ...
            var primativeRootPtr = (byte*) (&primativeRootValue);
            var generationSeedPtr = (byte*) (&generationSeedValue);
            ...
            for (int index = 0; index < 0x100; index++)
            {
                ...
                primativeRootPtr[0] = (byte) ((primativeRootPtr[1] + (byte) (primativeRootPtr[0]
                                                                             *primativeRootPtr[2]))*primativeRootPtr[0] +
                                              primativeRootPtr[3]);
                generationSeedPtr[0] = (byte) ((generationSeedPtr[1] - (byte) (generationSeedPtr[0]
                                                                               *generationSeedPtr[2]))*
                                               generationSeedPtr[0] + generationSeedPtr[3]);
            }
        }
        ...
        public void GenerateKeys(int account, int authentication)
        {
            ...
            var generationSeed = (uint) (((account + authentication) ^ 0x4321) ^ authentication);
            uint squaredSeed = generationSeed*generationSeed;
            var generationSeedPtr = (byte*) (&generationSeed);
            var squaredSeedPtr = (byte*) (&squaredSeed);
            ...
            if (_keyVector != null)
                lock (_decryptionLock)
                    for (int index = 0; index < length; index++)
                    {
                        ...
                        result[index] ^= (_keyVector[(byte) (_decryptionIncrementor & 0xFF)]);
                        result[index] ^= (_keyVector[(byte) (_decryptionIncrementor >> 8) + 0x100]);
                ...
                lock (_decryptionLock)
                    for (int index = 0; index < length; index++)
                    {
                        result[index] = (byte) (buffer[index] ^ 0xAB);
                        result[index] = (byte) (result[index] >> 4 | result[index] << 4);
                        result[index] ^= (_iv[(byte) (_decryptionIncrementor & 0xFF)]);
                        result[index] ^= (_iv[(byte) (_decryptionIncrementor >> 8) + 0x100]);
                        _decryptionIncrementor++;
                    }
            return result;
        }
        ...
        /// <param name="position">The position to begin writing data to in the pointed packet</param>
            ...
            if (_keyVector != null)
                lock (_decryptionLock)
                    for (int index = position; index < length + position; index++)
                    {
                        ...
                        packet[index] ^= (_keyVector[(byte) (_decryptionIncrementor & 0xFF)]);
                        packet[index] ^= (_keyVector[(byte) (_decryptionIncrementor >> 8) + 0x100]);
                        _decryptionIncrementor++;
                    }
            else
                lock (_decryptionLock)
                    for (int index = position; index < length + position; index++)
                    {
                        packet[index] = (byte) (buffer[index - position] ^ 0xAB);
                        packet[index] = (byte) (packet[index] >> 4 | packet[index] << 4);
                        packet[index] ^= (_iv[(byte) (_decryptionIncrementor & 0xFF)]);
                        packet[index] ^= (_iv[(byte) (_decryptionIncrementor >> 8) + 0x100]);
                        _decryptionIncrementor++;
                    }
        }
        ...
        public byte[] Encrypt(byte[] packet, int length)
        {
            ...
            lock (_encryptionLock)
                for (int index = 0; index < length; index++)
                {
                    ...
                    result[index] = (byte) (result[index] >> 4 | result[index] << 4);
                    result[index] ^= (_iv[(byte) (_encryptionIncrementor & 0xFF)]);
                    result[index] ^= (_iv[(byte) (_encryptionIncrementor >> 8) + 0x100]);
