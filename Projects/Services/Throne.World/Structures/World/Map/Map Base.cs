...
namespace Throne.World.Structures.World
{
    ...
    public abstract class MapBase
    {
        ...
        protected readonly Dictionary<Int32, Location> PortalDestination;
        protected readonly MapInfoRecord _record;

        public LogProxy Log;

        private Cell[,] _cells;

        protected MapBase(MapInfoRecord record)
        {
            _record = record;
                ...
                using (var BR = new BinaryReader(File.OpenRead(DataMapPath)))
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            this[x, y].AddFlag(baseType).SetArgument(cellHeight);
                        }
